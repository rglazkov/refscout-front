...
/**
 * What the checks are proposed from (M1.4.2). The table of signals is §4 of the
 * specification, and it is read from the text rather than from the extension:
 * a file called `notes.txt` that begins with `@article{` is a bibliography, and
 * a `.tex` without `\documentclass` is not a manuscript.
 *
 * Nothing here forbids anything. Every check is available on every document -
 * the automatic proposal decides what to suggest and never what to allow (§4).
 * A row of the table only says which boxes start ticked.
 */
const BIBTEX_ENTRY =
...
export function proposeChecks(text: string, format: SourceFormat): readonly ModuleId[] {
  ...
  // and the person says on the card what the document is for (§4).
