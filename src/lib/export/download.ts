...
/**
 * One mechanism for every download in the product. Text into a Blob with an
 * explicit type, a link carrying `download`, and the object URL released
...
/**
 * The text of a document, in the extension it arrived under. While every format
 * the buffer holds is text, "downloads in the format it was brought in" is
 * literally true; PDF and Word are the exceptions, and they lose them once the
 * browser can build those files again.
 */
export function downloadDocumentText(
  ...
  // so what is saved differs from what was opened in nothing at all.
  const body =
