...
/**
 * The working screen (§4). Four zones from top to bottom in the order the
 * person acts in - intake, buffer, plan, run - and they do not replace one
 * another: the drop zone stays on screen while the buffer fills, and the list
 * stays where it is when the next document is added.
 *
 * The whole of the working area is replaced by exactly one thing in this
 * milestone: a run that is under way, which then becomes the results without
 * the screen being rebuilt (§8).
 */
export default function Workspace() {
  // One client for the tab. Server state belongs to Query and browser state to
  // Zustand, and the boundary is not negotiable (M1.2.3).
      ...
      new QueryClient({
        defaultOptions: {
          queries: {
            ...
            // has made its decision and repeating will not change it (M1.8.3).
            retry: false,
  ...
  // before the mock was intercepting would go to the real address (M1.7.6).
...
function WorkspaceBody() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => {
      ...
      // there is no result yet, so there is nothing to send again (§9).
      resetJob();
    },
  });

  // A failed module is re-run inside the job that already exists: the text is
  // not sent again, and no new job is created from this screen (M1.8.6, §9).
  const retry = useMutation({
    mutationFn: (input: { readonly module: ModuleId }) =>
        ...
        <>
          ...
          <Collapse open={items.length > 0}>
            ...
            <LaunchRow
              ...
              pending={run.pending}
              failure={run.failure}
              onRun={(sending, options) => run.run(sending, options)}
            />

            {/* Said plainly, because it is true in this milestone: the buffer
                lives as long as the tab does. Storage that survives a reload is
                M4, and until it exists the honest sentence is the feature (§6). */}
            <p
        ...
        <div className="mt-6">
          ...
          {job.status.state === "failed" ? (
            /* A job that failed as a whole gets its own screen: the code, the
               ...
               there is nothing to repeat under its key (§8). */
            <div
              role="alert"
            ...
            >
              ...
              <p className="mt-1 text-sm">{t("failedBody", { jobId: job.status.id })}</p>
              <Button
                type="button"
                variant="outline"
                size="sm"
            ...
            <ResultsScreen
              ...
              running={running}
              onNewCheck={newCheck}
              onRetryModule={(_docId, module) => retry.mutate({ module })}
            />
          )}
        </div>
      )}
