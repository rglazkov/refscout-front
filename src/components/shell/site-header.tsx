...
/**
 * Features and Pricing, plus the theme toggle and the account entry (M0.6.1).
 * There are no per-tool tabs here and none will appear at any stage: the
 * product is one checker with several built-in capabilities, not a row of
 * separate tools.
 *
 ...
 * (`--nav-foreground`); the dark theme has no room above them without turning
 * them into body text, so it keeps a denser tint instead.
 */
export function SiteHeader({ locale }: { readonly locale: Locale }) {
  const t = useTranslations("nav");

  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/20 backdrop-blur-md dark:bg-background/60">
      <div className="mx-auto flex h-14 max-w-6xl items-center gap-2 px-3 nav:gap-4 nav:px-4">
        ...
        {/* The language switcher belongs here, beside the theme (§15). It is
            not mounted while there is one language: a menu with a single row
            is a control that asks a question with one answer. The parts it
            needs are in place - the locale list, `unlocalizedPath` and its
            words in the dictionary - so it arrives with the second language
