...
import { type ModuleResult } from "@/lib/domain";
import {
  anchoringOf,
  forgetWording,
  reportAnchoring,
...
describe("a body is trusted only when it was counted over the text we sent", () => {
  ...
  it("a hash the server recomputed differently takes the places away", () => {
    sendBoth();
    recordSnapshot(OWN.docId, { textSha256: "0".repeat(64), cpLength: OWN.cpLength });
...
describe("a place is said in words, and the words are ours", () => {
  ...
  beforeEach(() => {
    ...
    docRegistry.put(docId, {
      ...
      originalSha256: "",
      pages: [
        { page: 4, from: 12_000, to: 13_000 },
        { page: 9, from: 38_000, to: 39_000 },
      ],
      hadBom: false,
      eol: "\n",
    });
  ...
  it("a finding cited twice names both pages and counts its places", () => {
    ...
    const places = placesOf(docId, issue?.anchors ?? [], { anchored: true });
  ...
  it("a body that cannot be trusted keeps the key and the quote and loses the pages", () => {
    const issue = parsed().issues[0];
    const places = placesOf(docId, issue?.anchors ?? [], { anchored: false });
    expect(places.pages).toEqual([]);
    expect(places.bibkeys).toEqual(["smith2019attention"]);
    expect(places.quote).toBe("Smith et al. [22]");
  });

  it("a range whose quote is not its length gives up its page", () => {
    // Truncated, or measured in another unit; either way the number beside it
    // describes a different text.
    const result = parsed({
      issues: [
        {
          ...body.issues[0],
    ...
    expect(
      placesOf(docId, result.issues[0]?.anchors ?? [], { anchored: true }).pages,
    ).toEqual([]);
